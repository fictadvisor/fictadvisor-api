  ...
  async parseHtmlWeek (week, group, dom) {
    ...
    for (let i = 1; i < table.rows.length; i++) {
      ...
      for (let j = 2; j < row.childNodes.length; j++) {
        ...
        const teacherLinks = Object.values(td.childNodes).filter((n) => !(n instanceof dom.window.HTMLBRElement || n instanceof dom.window.Text || n instanceof dom.window.HTMLSpanElement || n.href.match(domainRegex)?.includes('http://maps.google.com'))); 

        for (let k = 0; k < teacherLinks.length; k++) {
          const teacherString = teacherLinks[k].getAttribute('title').split(' ');
          ...
          if (teacherLinks.length > localRowPairs.length && k === teacherLinks.length - 1 - 1) {
            ...
            const secondaryTeacher = parseTeacherName(teacherLinks[k + 1].getAttribute('title').split(' '));
