...
describe('ScheduleService', () => {
  ...
  describe('getGeneralGroupEvents', () => {
    ...
    it('should return only lectures, practices and laboratories events', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-05T00:01:00'));
      const schedules = [
        await scheduleService.getGeneralGroupEvents('group', 1),
        await scheduleService.getGeneralGroupEvents('group', 2),
        await scheduleService.getGeneralGroupEvents('group', 3),
        await scheduleService.getGeneralGroupEvents('group', 4),
        await scheduleService.getGeneralGroupEvents('group', 5),
      ];
      for (const schedule of schedules) {
        expect(schedule.events.length).not.toBe(0);
        expect(schedule.events.every(
  ...
  describe('getEvent', () => {
    it('should return all information about event and discipline with period', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-05T00:01:00'));
      const { event, discipline } = await scheduleService.getEvent('lecture-event-1st-semester-every-fortnight-09-05', 1);
      expect({
        id: event.id,
        startTime: event.startTime,
        endTime: event.endTime,
      ...
      expect(event.eventInfo.length).not.toBe(0);
      expect(event.lessons.length).not.toBe(0);
  ...
  describe('getAllGroupEvents', () => {
    ...
    it('should return events without practices', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-05T00:01:00'));
      const events = await scheduleService.getAllGroupEvents('group', 2, {
        addPractice: false,
      });
      expect(
        events.every(
          (event) => event.lessons.every(
            (lesson) => lesson.disciplineType.name !== DisciplineTypeEnum.PRACTICE
          )
        )
      ).toBe(true);
    });

    it('should return events without any disciplineType', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-05T00:01:00'));
      const events = await scheduleService.getAllGroupEvents('group', 2, {
        addLecture: false,
        addPractice: false,
  ...
  describe('getGroupEvents', () => {
    it('should return all group events in the specified week', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-12T00:01:00'));
      const { startTime, events, week } = await scheduleService.getGroupEvents('user', 'group', 2, {});
      expect(events.length).not.toBe(0);
    ...
    it('should return general events for another group', async () => {
      jest.useFakeTimers().setSystemTime(new Date('2022-09-12T00:01:00'));
      const { events } = await scheduleService.getGroupEvents('user', 'anotherGroup', 2, {});
      
      expect(
        events.every((event) => event.groupId === 'anotherGroup')
      ).toBe(true);
